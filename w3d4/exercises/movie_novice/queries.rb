...
def below_average_years
  ...
  Movie.select('yr, COUNT(title) as bad_movies').where('score < ?', 5).group(:yr)
end
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins("INNER JOIN castings ON castings.actor_id = actors.id")
        .joins("INNER JOIN movies ON castings.movie_id = movies.id")
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins("INNER JOIN castings ON castings.movie_id = movies.id")
        .joins("INNER JOIN actors ON castings.actor_id = actors.id")
